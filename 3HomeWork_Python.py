...
print('Стандартные валюты: usd, chf, byn, eur, uah')
options = str(input('Введите нужную вам валюту из предложенных или введите свою: '))
target_currency_amount = int(input('Введите количество денег, которые вы собираетесь перевести в другую валюту: '))
if (isinstance(target_currency_amount, int)) and target_currency_amount >= 0:
    if options == 'usd':
        currency_result = target_currency_amount * usd_usd_rate
        print(target_currency_amount, usd_item, '=', currency_result, usd_item)
    elif options == 'chf':
        ...
        if options != '':
            usd_new_rate = float(input('Введите коэффициент перевода: '))
            currency_result = target_currency_amount * usd_new_rate
            print(target_currency_amount, usd_item, '=', currency_result, options)        
else:
    print('Введено неккоректное значение количества денег')
